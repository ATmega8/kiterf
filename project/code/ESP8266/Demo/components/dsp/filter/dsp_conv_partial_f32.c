 ...
 * If the requested subset does not fall in this range then the functions return dsp_MATH_ARGUMENT_ERROR.
 * Otherwise the functions return dsp_MATH_SUCCESS.
 * \note Refer dsp_conv_f32() for details on fixed point behavior.
 *
 *
 * <b>Fast Versions</b>
 *
 ...
 * @return  Returns either dsp_MATH_SUCCESS if the function completed correctly or dsp_MATH_ARGUMENT_ERROR if the requested subset is not in the range [0 srcALen+srcBLen-2].
 */

dsp_status dsp_conv_partial_f32(
    float32_t *pSrcA,
    uint32_t srcALen,
    float32_t *pSrcB,
    uint32_t srcBLen,
    ...
    uint32_t numPoints)
{
    ...
    dsp_status status;                             /* status of Partial convolution */

    /* Check for range of output samples to be calculated */
    if ((firstIndex + numPoints) > ((srcALen + (srcBLen - 1U)))) {
        /* Set status as dsp_ARGUMENT_ERROR */
        ...
        /* set status as dsp_SUCCESS as there are no argument errors */
